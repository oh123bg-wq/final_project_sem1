...
// 1. Ensure user is logged in (确保用户已经登录)
...
// 2. Check if form is submitted via POST (检查是否通过 POST 提交)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['card_id'])) {
    $card_id = intval($_POST['card_id']);

    // 3. Check if the card already exists in user's collection (检查卡牌是否存在)
    ...
    if ($existing_record) {
        // 4. Card exists -> Update quantity + 1 (卡牌存在，数量 + 1)
        ...
        // 5. Card does not exist -> Insert new record (卡牌不存在，新插入一条)
    ...
    // 6. Redirect back to browse page with success parameter (成功后重定向)
